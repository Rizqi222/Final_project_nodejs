import BorrowsModel from "../models/BorrowsModel.js";
...
export const addBorrows = async (req, res) => {
  ...
  const borrows = await BorrowsModel.create(data);
...
export const getOneBorrows = async (req, res) => {
    ...
    let borrows = await BorrowsModel.findByPk(id);
    res.status(200).send(borrows);
  };
  ...
  export const updateBorrows = async (req, res) => {
    ...
    const borrows = await BorrowsModel.update(req.body, { where: { id: id } });
  ...
  await BorrowsModel.destroy({ where: { id: id } });
